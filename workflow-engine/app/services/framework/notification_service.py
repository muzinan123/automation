...
class NotificationService():
    ...
    @staticmethod
    def list_by_user(user_id, query=None, created_at=None, readed=None):
        ns = Notification.query.filter(Notification.target_user_id == user_id,
                                       Notification.readed == readed if readed is not None else "",
    ...
    def add(content, type, targets):
        for target_user_id in targets:
            ns = Notification(content=content, type=type, target_user_id=target_user_id, created_at=datetime.datetime.now())
            db.session.add(ns)
        db.session.commit()

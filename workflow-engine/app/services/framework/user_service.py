...
class UserService(object):
    ...
    @staticmethod
    def get_users(query, role_id=None, enabled=None):
        u = User.query.filter(or_(User.name.like("%" + query + "%") if query is not None else "",
                              ...
                              User.role_id == role_id if role_id else "",
                              User.enabled == enabled if enabled is not None else "")
        return u
    ...
    def get_user_id(query):
        user = User.query.filter(or_(User.name == query if query else "",
                                  User.real_name == query if query else "")).first()
        if user:
            return user.id

    #根据ID集合，查询对应的人员信息
    @staticmethod
    def get_users_ids(ids):
        users = User.query.filter(User.id.in_(ids) if ids else "")
        return users
